...
require('./VANZZ.js')
nocache('./VANZZ.js', module => console.log(`${module} telah di update !!`))
...
const starts = async (herman = new WAConnection()) => {
    ...
    herman.on('chat-update', async (message) => {
        require('./VANZZ.js')(herman, message, _welkom)
    })
...
if (anu.action == 'add' && !mem.includes(herman.user.jid)) {
            ...
            if (anu.action == 'remove' && !mem.includes(herman.user.jid)) {
                ...
                goodbyeButt = { contentText: `${out}`, footerText: `*${time_wel} -  ${time_welc}* \n© _VANZZ BOTZ_ 2022`, buttons: goodbyeBut, headerType: 6, locationMessage: bosco3.message.locationMessage}
...
function nocache(module, cb = () => { }) {
    console.log('Module', `'${module}'`, 'Subscribe VANZZ OFC')
